...
if [ -d "docs/endless-ts-sdk-$npm_package_version" ]; then
  echo "WARNING! Docs folder already exists, overwriting docs for version $npm_package_version";
fi

npx typedoc src/index.ts --out "docs/endless-ts-sdk-$npm_package_version" --cleanOutputDir --excludeInternal --includeVersion;
...
if [ -n "$LINE" ]; then
  ...
  # TODO: Make this more stable / a different way of generating that isn't just inserting a line
  {
    ...
    echo "- [$LABEL - endless-ts-sdk-$npm_package_version](endless-ts-sdk-$npm_package_version)";
    tail -n +$NEXT_LINE $INDEX_FILE;
  } > $INDEX_FILE.tmp && mv $INDEX_FILE.tmp $INDEX_FILE
fi

# Now update the redirect
REDIRECT_FILE='docs/endless-ts-sdk-latest/index.md';
$(sed -i.bak "s/redirect_to:.*/redirect_to: \/@endless-labs\/ts-sdk-${npm_package_version}/" $REDIRECT_FILE)
